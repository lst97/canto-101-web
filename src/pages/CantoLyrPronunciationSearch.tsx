...
export default function CantoLyrPronunciationSearch(): ReactElement {
  ...
  type NumericRowKey = "tone1" | "tone2" | "tone3" | "tone4" | "tone5" | "tone6";

  const toneRows: Array<{ toneIndex: string; mnemonicDigit: string; key: NumericRowKey }> = [
  ...
  return (
    <main className="mx-auto flex max-w-4xl flex-col gap-8 px-6 pb-24 pt-16 md:px-10 md:pt-20">
      <div className="flex flex-col gap-6">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>{t("cantoLyr.pages.pron.heading")}</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
      ...
      <section className="space-y-4" aria-label={t("cantoLyr.pages.pron.numericHeading")}>
          ...
          <AccordionItem value="tones">
            ...
            <AccordionContent className="space-y-4">
              ...
              <div className="overflow-hidden rounded-xl border border-border/80 bg-muted/40">
                <Table>
                  <TableCaption>{t("cantoLyr.pages.pron.numericTable.caption")}</TableCaption>
                  <TableHeader>
                    <TableRow className="bg-muted/70 text-foreground">
                      <TableHead className="text-foreground">
                        {t("cantoLyr.pages.pron.numericTable.columns.toneIndex")}
                      </TableHead>
                      <TableHead className="text-foreground">
                        {t("cantoLyr.pages.pron.numericTable.columns.mnemonicDigit")}
                      </TableHead>
                      <TableHead className="text-foreground">
                  ...
                  <TableBody>
                    {toneRows.map(({ toneIndex, mnemonicDigit, key }, index) => (
                      <TableRow
                        key={toneIndex}
                        className={cn(
                      ...
                      >
                        <TableCell className="font-semibold text-foreground">{toneIndex}</TableCell>
                        <TableCell className="font-medium text-foreground/90">{mnemonicDigit}</TableCell>
                        <TableCell>{t(`cantoLyr.pages.pron.numericTable.rows.${key}.toneName`)}</TableCell>
                        <TableCell>{t(`cantoLyr.pages.pron.numericTable.rows.${key}.example`)}</TableCell>
