  ...
  return (
    <header id="hero" className="relative">
      ...
      <div className="mt-10 flex flex-wrap gap-4">
        ...
        <Button size="lg" variant="outline" className="rounded-full border-2 border-muted text-base font-semibold">
          {t('homepage.hero.secondaryCta')}
        </Button>
      </div>
      <dl className="mt-12 grid gap-6 sm:grid-cols-3">
        {heroMetricKeys.map((key) => (
          <Card key={key} className="rounded-3xl border border-border/60 bg-card/80 p-6 text-left backdrop-blur-sm">
