...
const productItems = [
  {
    ...
    featureKeys: ['romanization', 'vocabulary', 'phrases', 'grammar', 'pronunciation'] as const,
  },
  {
...
export default function ProductsShowcase(): ReactElement {
  ...
  const [activeProductKey, setActiveProductKey] = useState<ProductKey>(productItems[0].translationKey)
  const productTabRefs = useRef<Array<HTMLButtonElement | null>>([])

  const handleProductKeyDown = (event: KeyboardEvent<HTMLButtonElement>, index: number): void => {
    ...
    if (event.key === 'Home') nextIndex = 0
      ...
      nextIndex = (index + direction + productItems.length) % productItems.length
    }
  ...
  return (
    <section id="products" className="mt-24 rounded-[2.5rem] border border-border/80 bg-card/70 px-8 py-12 backdrop-blur-sm md:px-12">
      <div className="flex flex-col gap-12 lg:grid lg:grid-cols-[minmax(0,18rem)_1fr] lg:items-start">
        <div className="space-y-8 lg:max-w-sm">
          ...
          <div role="tablist" aria-label={t('homepage.products.title')} aria-orientation="horizontal" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
              ...
              >
                <Card className="h-full rounded-[2.5rem] border border-border/80 bg-background/95 p-10 shadow-[0_32px_80px_-56px_rgba(0,0,0,0.5)] backdrop-blur">
                  <CardHeader className="space-y-5 p-0">
                    <Badge variant="secondary" className="w-fit rounded-full border-0 bg-primary/10 px-4 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-primary">
                      {t('homepage.products.title')}
                    </Badge>
                  ...
                  <CardContent className="p-0">
                    <ul className="mt-8 grid gap-4 text-base text-foreground sm:grid-cols-2 sm:gap-6">
                      {product.featureKeys.map((featureKey) => (
                        <li key={`${product.translationKey}.features.${featureKey}`} className="flex items-start gap-3 rounded-2xl bg-primary/5 px-5 py-4">
                          <span className="mt-1 inline-flex size-2.5 flex-none rounded-full bg-primary" aria-hidden />
                          <span className="leading-relaxed text-foreground">
                            {t(`${product.translationKey}.features.${featureKey}`)}
                          </span>
                        </li>
                      ))}
                    </ul>
