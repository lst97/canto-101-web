...
import { AiLyricPronunciationSearch } from '../components/cantoLyr/lyrics-search/index.ts';
...
const tipKeys = ['digits', 'position', 'filters', 'aiQuery'] as const;

export default function CantoLyrAiLyricPronunciationSearch(): ReactElement {
  ...
  return (
    <main className="mx-auto flex max-w-4xl flex-col gap-8 px-6 pb-24 pt-16 md:px-10 md:pt-20">
      <div className="flex flex-col gap-6">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbItem>
              <BreadcrumbPage>
                {t('cantoLyr.ai.lyricSearch.pron.nav')}
              </BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <h1 className="text-3xl font-semibold tracking-tight">
                {t('cantoLyr.ai.lyricSearch.pron.heading')}
              </h1>
            ...
            <p className="text-muted-foreground max-w-prose">
              {t('cantoLyr.ai.lyricSearch.pron.description')}
            </p>
          </div>
      ...
      <section
        aria-label={t('cantoLyr.ai.lyricSearch.pron.tipsHeading')}
        className="space-y-4"
      >
          ...
          <CardContent className="space-y-3 p-6">
            <div className="space-y-1">
              <h2 className="text-lg font-semibold tracking-tight">
                {t('cantoLyr.ai.lyricSearch.pron.tipsHeading')}
              </h2>
              <p className="text-sm text-muted-foreground">
                {t('cantoLyr.ai.lyricSearch.pron.tipsDescription')}
              </p>
            </div>
            <ul className="space-y-2 text-sm text-muted-foreground">
              {tipKeys.map(key => (
                <li key={key}>
                  {t(`cantoLyr.ai.lyricSearch.pron.tips.${key}`)}
                </li>
              ))}
            </ul>
          </CardContent>
      ...
      <section
        aria-label={t('cantoLyr.ai.lyricSearch.pron.heading')}
        className="space-y-4"
      >
        <div className="space-y-2">
          <h2 className="text-xl font-semibold tracking-tight">
            {t('cantoLyr.ai.lyricSearch.pron.formHeading')}
          </h2>
          <p className="text-sm text-muted-foreground max-w-prose">
            {t('cantoLyr.ai.lyricSearch.pron.formDescription')}
          </p>
        </div>
        <QueryErrorBoundary>
          <AiLyricPronunciationSearch
