...
export interface UseAiLexiconSearchResult {
  ...
  loading: boolean;
  error: string | null;
  search: (override?: { q: string; pronunciation: string; limit?: string }) => Promise<void>;
  reset: () => void;
}
...
export function useAiLexiconSearch(): UseAiLexiconSearchResult {
  ...
  const mutation = useMutation<AiLexiconSearchResponse, AppError, AiLexiconSearchQuery>({
  ...
  const search = useCallback(async (override?: { q: string; pronunciation: string; limit?: string }) => {
    ...
    if (!parsed.success) {
      setError(parsed.error.issues[0]?.message ?? "cantoLyr.errors.lexicon.missingQuery");
      return;
    }
