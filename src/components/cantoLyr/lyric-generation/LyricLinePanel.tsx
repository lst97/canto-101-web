...
import type { LyricLineResult } from "@/lib/schemas/lyric-generation";
...
export function LyricLinePanel({ line }: LyricLinePanelProps) {
  ...
  const candidateStats = useMemo(() => ([
    { label: t("cantoLyr.ai.lyrics.lines.candidateStats.total"), value: line.candidatePoolStats.total },
    { label: t("cantoLyr.ai.lyrics.lines.candidateStats.semantic"), value: line.candidatePoolStats.semanticCount },
    { label: t("cantoLyr.ai.lyrics.lines.candidateStats.freqTop"), value: line.candidatePoolStats.freqTopCount },
    { label: t("cantoLyr.ai.lyrics.lines.candidateStats.freqRandom"), value: line.candidatePoolStats.freqRandomCount },
    ...
    return (
      <div className="flex flex-wrap items-center gap-2">
        {line.digitSet.map(digit => (
          <Badge key={`${line.lineIndex}-${digit}`} variant="secondary" className="font-mono text-xs">
            {digit}
          </Badge>
        ))}
      </div>
      ...
      <div className="space-y-2">
        <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
          <span className="rounded-md bg-muted/60 px-2 py-0.5 font-mono">
            {t("cantoLyr.ai.lyrics.lines.toneSequence", { tone: line.toneSequence })}
          </span>
          <span className="rounded-md bg-muted/60 px-2 py-0.5 font-mono">
            {t("cantoLyr.ai.lyrics.lines.lineIndex", { index: line.lineIndex + 1 })}
          </span>
        </div>
        {digitBadges && (
          ...
          <blockquote className="rounded-lg border-l-4 border-primary/70 bg-muted/30 p-3 text-foreground">
            ...
            <footer className="mt-2 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
              <span className="rounded-md bg-muted/70 px-2 py-0.5 font-mono">
                {t("cantoLyr.ai.lyrics.lines.primaryPattern", { pattern: primarySentence.patternId })}
              </span>
              <span className="rounded-md bg-muted/70 px-2 py-0.5 font-mono">
                {t("cantoLyr.ai.lyrics.lines.primaryRank", { rank: primarySentence.finalRank.toFixed(2) })}
              </span>
              <span className="rounded-md bg-muted/70 px-2 py-0.5 font-mono">
                {t("cantoLyr.ai.lyrics.lines.primaryMmr", { score: primarySentence.mmrScore.toFixed(3) })}
              </span>
            </footer>
          </blockquote>
        ) : (
        ...
        {line.warnings.length > 0 && (
          <div className="flex flex-wrap items-center gap-2" role="status">
            {line.warnings.map((warning, idx) => (
              <Badge key={`${line.lineIndex}-warning-${idx}`} variant="outline" className="text-[11px] uppercase tracking-wide text-amber-600">
                {warning}
              </Badge>
            ))}
          </div>
      ...
      <section className="space-y-3">
        ...
        <dl className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {candidateStats.map(stat => (
            <div key={stat.label} className="rounded-lg border border-border/50 bg-muted/20 p-3">
      ...
      <section className="space-y-3">
        ...
        {line.topSentences.length > 0 ? (
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>{t("cantoLyr.ai.lyrics.lines.topSentences.columns.text")}</TableHead>
            ...
            <TableBody>
              {line.topSentences.map((sentence, idx) => (
                <TableRow key={`${line.lineIndex}-sentence-${idx}`} className={idx === 0 ? "bg-muted/40" : undefined}>
        ...
        {line.patterns.length > 0 ? (
          <div className="grid gap-3 lg:grid-cols-2">
            {line.patterns.map(pattern => (
              <div key={pattern.id} className="rounded-lg border border-border/50 bg-muted/20 p-3">
                <div className="flex items-center justify-between gap-3">
                  <span className="text-sm font-semibold text-foreground">{pattern.patternString}</span>
                ...
                {pattern.groups.length > 0 && (
                  <p className="mt-2 text-xs text-muted-foreground">
                    {t("cantoLyr.ai.lyrics.lines.patterns.groups", { groups: pattern.groups.join(" · ") })}
                  </p>
                )}
              </div>
            ))}
          ...
          <div className="max-h-60 overflow-y-auto rounded-lg border border-border/40 bg-muted/10 p-3">
            <ul className="space-y-2">
              {line.allLineCandidates.map(candidate => (
                <li key={`${line.lineIndex}-${candidate.patternId}-${candidate.text}`} className="space-y-1">
                  <p className="text-sm text-foreground">{candidate.text}</p>
                  <p className="text-xs text-muted-foreground">
                    {t("cantoLyr.ai.lyrics.lines.allCandidates.pattern", { pattern: candidate.patternId })}
                  </p>
                </li>
              ))}
            </ul>
        ...
        <section className="space-y-3">
          ...
          <ul className="space-y-2">
            {additionalSentences.map((sentence, idx) => (
              <li key={`${line.lineIndex}-additional-${idx}`} className="rounded-md border border-border/50 bg-muted/20 p-3">
                <p className="text-sm text-foreground">{sentence.text}</p>
                <p className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                  <span className="rounded-md bg-muted/60 px-2 py-0.5 font-mono">
                    {sentence.patternId}
                  </span>
                  <span>{t("cantoLyr.ai.lyrics.lines.additional.rank", { rank: sentence.finalRank.toFixed(2) })}</span>
                  <span>{t("cantoLyr.ai.lyrics.lines.additional.mmr", { score: sentence.mmrScore.toFixed(3) })}</span>
                </p>
              </li>
            ))}
          </ul>
        ...
        <section className="space-y-3">
          ...
          <ol className="space-y-2">
            {line.topParagraphCandidates.map((candidate, idx) => (
              <li key={`${line.lineIndex}-paragraph-${idx}`} className="rounded-md border border-border/50 bg-muted/15 p-3">
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  {t("cantoLyr.ai.lyrics.lines.paragraphs.label", { index: idx + 1 })}
                </p>
