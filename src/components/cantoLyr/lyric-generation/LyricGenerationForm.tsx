...
import type { LyricSessionOptions } from "@/hooks/useLyricGeneration";
...
export function LyricGenerationForm({ loading, onSubmit, serverError }: LyricGenerationFormProps) {
  ...
  const validatePrompt = useCallback((value: string): string | undefined => {
    ...
    if (trimmed.length > PROMPT_MAX) return "cantoLyr.errors.lyrics.promptTooLong";
    return undefined;
  }, []);

  const validateSequences = useCallback((value: string): string | undefined => {
    const sequences = parseToneSequences(value);
    if (sequences.length === 0) return "cantoLyr.errors.lyrics.missingSequences";
  ...
  const validateSeed = useCallback((value: string): string | undefined => {
    ...
    try {
      ...
      if (!Number.isSafeInteger(num) || num < 0) return "cantoLyr.errors.lyrics.invalidSeed";
    } catch {
  ...
  const validateTop = useCallback((value: string): string | undefined => {
    ...
    const num = Number(trimmed);
    if (!Number.isInteger(num) || num < 1 || num > 3) return "cantoLyr.errors.lyrics.invalidTop";
    return undefined;
  }, []);
  ...
  return (
    <Card className="border-border/60 shadow-none">
      <CardHeader>
        ...
        <CardDescription>{t("cantoLyr.ai.lyrics.formDescription")}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        >
          ...
          >
            {(field) => {
              const showError = (
                ...
                && (field.state.meta.isTouched || field.state.meta.isDirty || submitAttempted || form.state.isSubmitting)
              );
              const errKey = field.state.meta.errors[0];
              return (
                <div className="space-y-2">
                  <Label htmlFor="lyric-generation-prompt">{t("cantoLyr.ai.lyrics.form.promptLabel")}</Label>
                  <Textarea
                    ...
                    aria-describedby={showError ? "lyric-generation-prompt-error" : undefined}
                  />
                  {showError && errKey && (
                    <p id="lyric-generation-prompt-error" className="text-xs text-destructive" role="alert">
                      {t(errKey)}
                    </p>
                  )}
                </div>
              ...
              return (
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onBlur={field.handleBlur}
                    onChange={(event) => field.handleChange(event.target.value)}
                    placeholder={t("cantoLyr.ai.lyrics.form.sequencesPlaceholder")}
                    aria-invalid={showError}
                    aria-describedby={showError ? "lyric-generation-tone-sequences-error" : undefined}
                  ...
                  {showError && errKey && (
                    <p id="lyric-generation-tone-sequences-error" className="text-xs text-destructive" role="alert">
                      {t(errKey)}
                    </p>
                  )}
                </div>
                ...
                return (
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      inputMode="numeric"
                      aria-invalid={showError}
                      aria-describedby={showError ? "lyric-generation-seed-error" : undefined}
                    />
                    <p className="text-xs text-muted-foreground">
                      {t("cantoLyr.ai.lyrics.form.seedCaption")}
                    </p>
                    {showError && errKey && (
                      <p id="lyric-generation-seed-error" className="text-xs text-destructive" role="alert">
                        {t(errKey)}
                      </p>
                    )}
                  </div>
                ...
                return (
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      inputMode="numeric"
                      aria-invalid={showError}
                      aria-describedby={showError ? "lyric-generation-top-error" : undefined}
                    />
                    <p className="text-xs text-muted-foreground">
                      {t("cantoLyr.ai.lyrics.form.topCaption")}
                    </p>
                    {showError && errKey && (
                      <p id="lyric-generation-top-error" className="text-xs text-destructive" role="alert">
                        {t(errKey)}
                      </p>
                    )}
                  </div>
          ...
          <div className="flex items-center gap-3">
            <Button type="submit" disabled={loading} className="px-6" onClick={() => setSubmitAttempted(true)}>
              {loading ? (
                <LoadingIndicator
                  size="sm"
        ...
        {resolvedServerError && (
          <p role="alert" className="text-sm text-destructive">
            {resolvedServerError.startsWith("cantoLyr.") || resolvedServerError.startsWith("errors.")
