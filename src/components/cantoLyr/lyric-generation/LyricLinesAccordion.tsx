...
import { LyricLinePanel } from "./LyricLinePanel";
...
export function LyricLinesAccordion({ lines }: LyricLinesAccordionProps) {
  ...
  const accordionItems = useMemo(() => lines.map(line => {
    const snippet = line.topSentences[0]?.text ?? t("cantoLyr.ai.lyrics.lines.previewFallback");
    return {
        ...
        <AccordionItem key={item.key} value={item.key}>
          <AccordionTrigger>
            ...
            {item.line.warnings.length > 0 && (
              <Badge variant="outline" className="ml-3 text-[11px] uppercase tracking-wide">
                {t("cantoLyr.ai.lyrics.lines.warningsCount", { count: item.line.warnings.length })}
              </Badge>
            )}
          </AccordionTrigger>
          <AccordionContent>
