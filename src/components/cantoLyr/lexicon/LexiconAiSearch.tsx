...
import { useAiLexiconSearch } from "@/hooks/useAiLexiconSearch";
import type { AiLexiconSearchItem } from "@/lib/schemas/lexicon";
...
function buildDetailEntries(
  ...
  const entries: DetailEntry[] = [
    { label: translate("cantoLyr.lexicon.details.jyutping"), value: formatList(item.jyutping) },
    { label: translate("cantoLyr.lexicon.details.tone"), value: item.tone },
    { label: translate("cantoLyr.lexicon.details.consonants"), value: formatList(item.consonants) },
    { label: translate("cantoLyr.lexicon.details.rhymes"), value: formatList(item.rhymes) },
  ];

  if (typeof item.syllables === "number") {
    entries.push({ label: translate("cantoLyr.lexicon.details.syllables"), value: item.syllables.toLocaleString() });
  }
  if (typeof item.freq === "number") {
    entries.push({ label: translate("cantoLyr.lexicon.details.frequency"), value: item.freq.toLocaleString() });
  }
  if (item.pos) {
    entries.push({ label: translate("cantoLyr.lexicon.details.pos"), value: item.pos });
  }
  if (item.register) {
    entries.push({ label: translate("cantoLyr.lexicon.details.register"), value: item.register });
  }
  if (item.gloss) {
    entries.push({ label: translate("cantoLyr.lexicon.details.gloss"), value: item.gloss, fullWidth: true });
  }
  if (item.source) {
    entries.push({ label: translate("cantoLyr.lexicon.details.source"), value: item.source, fullWidth: true });
  }
...
export function LexiconAiSearch(): ReactElement {
  ...
  const formSchema = z.object({
    query: querySchema,
    pronunciation: pronunciationSchema.min(1, { message: "cantoLyr.errors.lexicon.missingPronunciation" }),
    limit: z
      .string()
      .trim()
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      await search({ q: value.query, pronunciation: value.pronunciation, limit: value.limit });
    },
  });

  const resolvedError = useMemo(() => {
    if (!error) return null;
    return error.startsWith("cantoLyr.") || error.startsWith("errors.") ? t(error) : error;
  }, [error, t]);
          ...
          <form.Field
            ...
            children={(field) => (
              <div className="space-y-2">
                <Label htmlFor="ai-lexicon-query">{t("cantoLyr.ai.lexicon.form.queryLabel")}</Label>
                <Input
                  id="ai-lexicon-query"
                  ...
                  aria-describedby={field.state.meta.errors.length ? "ai-lexicon-query-error" : undefined}
                />
              ...
              children={(field) => (
                <div className="flex-1 space-y-2">
                  ...
                  <Input
                    ...
                    onBlur={field.handleBlur}
                    onChange={(e) => field.handleChange(e.target.value)}
                    placeholder={t("cantoLyr.ai.lexicon.form.pronunciationPlaceholder")}
                    inputMode="numeric"
                    pattern="[023459]*"
                    autoComplete="off"
                    aria-invalid={field.state.meta.errors.length > 0}
                    aria-describedby={field.state.meta.errors.length ? "ai-lexicon-pronunciation-error" : undefined}
                  />
                  {field.state.meta.errors[0] && (field.state.meta.isTouched || form.state.isSubmitting) && (
            ...
            <form.Field
              ...
              children={(field) => (
                <div className="sm:w-36 space-y-2">
                  <Label htmlFor="ai-lexicon-limit">{t("cantoLyr.ai.lexicon.form.limitLabel")}</Label>
                  <Input
                    id="ai-lexicon-limit"
                    value={field.state.value}
                    onBlur={field.handleBlur}
                    ...
                    autoComplete="off"
                    aria-invalid={field.state.meta.errors.length > 0}
                    aria-describedby={field.state.meta.errors.length ? "ai-lexicon-limit-error" : undefined}
                  />
                  {field.state.meta.errors[0] && (field.state.meta.isTouched || form.state.isSubmitting) && (
          ...
          <section className="space-y-4" aria-live="polite">
            <div className="flex flex-col gap-2 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
              <div className="flex flex-wrap items-center gap-2">
                {summary && <span className="font-semibold text-foreground">{summary}</span>}
                {result.fromCache && (
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm">
                <span className="rounded-md bg-muted/60 px-2 py-0.5 font-mono text-muted-foreground/90">
                  {t("cantoLyr.ai.lexicon.summary.query", { query: result.query })}
                </span>
                <span className="rounded-md bg-muted/60 px-2 py-0.5 font-mono text-muted-foreground/90">
                  {t("cantoLyr.ai.lexicon.summary.pronunciation", { pronunciation: result.pronunciation })}
                </span>
...
const AiLexiconActivePanel = memo(function AiLexiconActivePanel({ item }: AiLexiconActivePanelProps) {
  const { t } = useTranslation();
    ...
    <div className="space-y-4 rounded-lg border border-border/60 bg-primary/2 p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          <h3 className="text-lg font-semibold leading-tight text-foreground">{item.surface}</h3>
        ...
        <div className="flex flex-col items-end gap-2 text-xs text-muted-foreground">
          ...
          <Badge variant="secondary" className="uppercase tracking-wide">
            {t("cantoLyr.ai.lexicon.similarityBadge", { percentage: similarityPercentage })}
          </Badge>
        </div>
      </div>
      <dl className="grid gap-3 sm:grid-cols-2">
        {details.map(detail => (
          <div key={`${item.id}-${detail.label}`} className={detail.fullWidth ? "sm:col-span-2" : undefined}>
